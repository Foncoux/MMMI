...
{
    gsl_rng* r = gsl_rng_alloc(gsl_rng_mt19937);// Initialiser le générateur de nombres aléatoires
    ...
    {   
        ...
        {        
            ...
            X0[NB_PARAM_TOT*classe + PARAM_ID_X0_infect] = gsl_rng_uniform(r)*(4478.0/POP_TOT);
        
            X0[NB_PARAM_TOT*classe + PARAM_ID_DELTA] = gsl_rng_uniform(r);
            X0[NB_PARAM_TOT*classe + PARAM_ID_EPS] = gsl_rng_uniform(r);

            X0[NB_PARAM_TOT*classe + PARAM_ID_GAMMA] = gsl_rng_uniform(r)*(1-X0[NB_PARAM_TOT*classe + PARAM_ID_DELTA].todouble());
            X0[NB_PARAM_TOT*classe + PARAM_ID_R] = gsl_rng_uniform(r)*(1-X0[NB_PARAM_TOT*classe + PARAM_ID_EPS].todouble());

            for (int i = 0; i < NB_CONFINEMENT+1; i++)
            {
                X0[NB_PARAM_TOT*classe + PARAM_ID_BETA0 + i] = gsl_rng_uniform(r);
            }
            
        } 
    }while (! validation_parametres(X0));
...
bool validation_parametres(const NOMAD::Point p){
    ...
    for (size_t classe = 0; classe < NB_CLASSE_AGE; classe++)
    {
        ...
        for (int i = 0; i < NB_CONFINEMENT+1; i++)
        {
            if(p[NB_PARAM_TOT*classe + PARAM_ID_BETA0 + i].todouble() >1 || p[NB_PARAM_TOT*classe + PARAM_ID_BETA0 + i].todouble() <0){
                ok=false;  
                
            }
        }

        if(p[NB_PARAM_TOT*classe + PARAM_ID_DELTA].todouble()>1 || p[NB_PARAM_TOT*classe + PARAM_ID_DELTA].todouble()<0){
            ok=false;  
             
        }
        if(p[NB_PARAM_TOT*classe + PARAM_ID_GAMMA].todouble()>1 || p[NB_PARAM_TOT*classe + PARAM_ID_GAMMA].todouble()<0){
            ok=false; 
               
        }
        if(p[NB_PARAM_TOT*classe + PARAM_ID_EPS].todouble()>1 || p[NB_PARAM_TOT*classe + PARAM_ID_EPS].todouble()<0){
            ok=false;   
             
        }
        if(p[NB_PARAM_TOT*classe + PARAM_ID_R].todouble()>1 || p[NB_PARAM_TOT*classe + PARAM_ID_R].todouble()<0){
            ok=false;    
        }
        if(p[NB_PARAM_TOT*classe + PARAM_ID_DELTA].todouble() + p[NB_PARAM_TOT*classe + PARAM_ID_GAMMA].todouble()>1 || p[NB_PARAM_TOT*classe + PARAM_ID_EPS].todouble() + p[NB_PARAM_TOT*classe + PARAM_ID_R].todouble() >1){
            ok=false;
        }

        if(p[NB_PARAM_TOT*classe + PARAM_ID_X0_infect].todouble() < 0 || p[NB_PARAM_TOT*classe + PARAM_ID_X0_infect].todouble() > 4478/POP_TOT){
            ok=false;
        } 

        somme1 = somme1 + p[NB_PARAM_TOT*classe + PARAM_ID_X0_infect].todouble();
